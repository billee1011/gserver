 ...
 */
public class AgentServerBootstrap {
	private static final Logger logger = LoggerFactory.getLogger(AgentServerBootstrap.class);
	...
	static {
		try {
			 ...
			 */
			if(!DEBUG) {
				String jarPath = AgentServerBootstrap.class.getProtectionDomain().getCodeSource().getLocation().getFile();
...
//					System.out.println("  --server-type=arg (=" + daemon.getServer_type() +")\n\t\t\t\tType of server: simple, nonblocking, hsha, threaded-selector, thread-pool");
	...
	private static void loadProps(Boolean isDebug) {
		...
		try {
			if(DEBUG) {
				filePath = AgentServerBootstrap.class.getClass().getResource("/").getPath() + "config.properties";
			} else {
				String jarPath = AgentServerBootstrap.class.getProtectionDomain().getCodeSource().getLocation().getFile();
				jarPath = jarPath.substring(0, jarPath.lastIndexOf('/'));
				jarPath = jarPath.substring(0, jarPath.lastIndexOf('/') + 1) + "conf";
