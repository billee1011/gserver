...
public class LoginServerBootstrap {
	private static final Logger logger = LoggerFactory.getLogger(LoginServerBootstrap.class);
	...
	static {
		try {
			 ...
			 */
			if(!DEBUG) {
				String jarPath = LoginServerBootstrap.class.getProtectionDomain().getCodeSource().getLocation().getFile();
	...
	public static void main(String[] args) {
		...
		try {
			...
			logger.error("Login Server started fail!", e);
			System.exit(-1);
		}
	}
...
//					System.out.println("  --server-type=arg (=" + daemon.getServer_type() +")\n\t\t\t\tType of server: simple, nonblocking, hsha, threaded-selector, thread-pool");
	...
	private static void loadProps(Boolean isDebug) {
		...
		try {
			if(DEBUG) {
				filePath = LoginServerBootstrap.class.getClass().getResource("/").getPath() + "config.properties";
			} else {
				String jarPath = LoginServerBootstrap.class.getProtectionDomain().getCodeSource().getLocation().getFile();
				jarPath = jarPath.substring(0, jarPath.lastIndexOf('/'));
				jarPath = jarPath.substring(0, jarPath.lastIndexOf('/') + 1) + "conf";
